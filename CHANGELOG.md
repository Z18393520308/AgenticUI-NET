...
- 文档收敛为一份使用指南和一份开发文档：合并快速开始、协议、各控件指南、网关与交接资料，
  移除 12 份重复/过期说明；明确 0.6.1 与 0.7.0 能力分界，保留既有快速开始链接。
- 补齐 WPF/WinForms 完整接入示例、生命周期、配对、下拉容器修复与安全限制；README/NuGet
  说明改为入口摘要，解决方案和社区文件同步更新；许可证、贡献规范、安全策略及历史记录保留。
- 增加文档链接、JSON 示例、动作覆盖和 NuGet 示例编译检查，并纳入 CI；官网安装命令同步到 0.7.0。

### 下拉列表读取与选择

- 修复 WPF 带普通样式/隐式主题的下拉框在容器未生成时被 selectItem 误拦截：
  自动展开/定位并异步等待真实容器（预算 2 秒、可取消），兼容虚拟化选项，保留原生绑定与禁用保护。
- 准备过程中检测人工交互、改选、卸载及列表变化，同控件修改命令防重入，结束后清理临时展开状态；
  itemsVersion 不再因容器从未知变为已知可用状态而误失效，选择前仍实时检查禁用状态。
- 补充 WPF 容器准备、取消、禁用保护与核心数据版本回归测试。
...
- 将 AgenticUI.Core、AgenticUI.Remote、AgenticUI.Wpf / AgenticUI.WinForms 统一升级到 0.7.0，
  关闭旧进程后重新构建启动；不要混用旧 DLL 或覆盖本地同版本缓存包。
- 旧独立 Gateway 已移除。改为在应用入口初始化一次 AgenticApplicationHost，并用 agenticui.json
  配置开关；不要同时启动同名 Named Pipe 服务。网络与 UDP 发现默认关闭，写动作仍须显式授权。
- 不需要 HTTP.sys/netsh/手动证书部署，但仍需首次独立核验指纹和配对；UDP 发现不能替代信任验证。
- items.v1 以实际 capabilities 为准，读取 getItems 后优先使用 itemKey；selectItem 表示选择已有候选，
  不新增或修改业务数据源，不进行模糊匹配，也不绕过禁用状态与业务校验。
