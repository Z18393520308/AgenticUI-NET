...
> 开发分支最新变化：不再使用 HTTP.sys 或系统证书安装。默认网络认证改为首次指纹核验、
> 一次性码配对和本地持久凭据；两个 Demo 已接入。旧显式网络令牌仅作为兼容入口。
> 详细配置与生命周期见 [内嵌网关与配对](embedded-host.zh-CN.md)，尚未发布。
...
- 基础 WSS Gateway 已进入源码；企业身份、集中审计、网关编排、设备管理、LTS/SLA、
  行业适配和私有部署仍属于未来可选企业服务方向，不应被宣称为当前已交付功能。
- 详细条款以 `LICENSE`、`LICENSING.md` 和 `EDITIONS.md` 为准。

## 7. 当前发布基线

- 首个稳定版为 `v0.2.0`，当前发布版本为 `v0.6.1`，发布结果以 GitHub Actions 为准。
