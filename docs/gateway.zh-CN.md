...
当前未发布源码已修复默认值覆盖显式配置的问题：Development 和 localhost 都不会自行
开启发现，请明确配置 Discovery.Enabled。请求 ID 只在最近 2048 个请求的窗口内去重，
连接可长期使用；它不是跨连接或窗口外的业务幂等机制。认证握手最长 10 秒。
动态气泡仍由桌面控件渲染，Gateway 不生成文案，也不包含大模型。
